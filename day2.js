...
let amountSuccessful = 0;

let passwords = `15-16 m: mhmjmzrmmlmmmmmm
5-6 d: dcdddhzld
3-4 s: vqssdcbl
3-6 b: bbhbbbqbbb
10-11 q: cqqntqhqwwh
...
function doAllPasswords() {
  for (let i = 0; i < passwords.length; i++) {
    passwords[i] = passwords[i].split(" ");
    passwords[i][0] = passwords[i][0].split("-");
    passwords[i][1] = passwords[i][1][0];

    let passwordforfunction = passwords[i][2];
    let letterforfunction = passwords[i][1];
    let leastforfunction = passwords[i][0][0];
    let mostforfunction = passwords[i][0][1];

    workWithProtocol(passwordforfunction, letterforfunction, leastforfunction, mostforfunction);
...
function workWithProtocol(password, letter, least, most) {
  let amountofletter = 0;
  if (!password.includes(letter)) return;
  for (let i = 0; i < password.length; i++) {
    if (password[i].includes(letter)) amountofletter++;
  }
  if (amountofletter >= least && amountofletter <= most) {
    amountSuccessful++;
    console.log(`Success! There have been ${amountSuccessful} successes.`);
  } else {
    console.log("Password is not up to snuff.");
  }
}

console.log(amountSuccessful)